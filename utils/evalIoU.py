...
from utils.helpers.csHelper import *
#import labels
...
if CSUPPORT:
    try:
        from utils.helpers import addToConfusionMatrix
    except:
        CSUPPORT = False
...
def evaluatePairPytorch(prediction, groundtruth, confMatrix, perImageStats, args):
    ...
    predictionNp = np.array(predictionImg).astype(np.uint8)

    groundTruthImg = groundtruth
    groundTruthNp = np.array(groundTruthImg).astype(np.uint8)

 
    # Check for equal image sizes
    if (predictionImg.shape[0] != groundTruthImg.shape[0]):
        printError("Image widths are not equal.")
    if (predictionImg.shape[1] != groundTruthImg.shape[1]):
