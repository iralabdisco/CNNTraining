...
def overlay_images(names, input, output, starting_index, runs="", tensor=True, convert_id = False):
    ...
    for i in range(mask.shape[0]):
        ...
        im.save('./CITYSCAPES/PSP/inference/'+str(runs)+str(starting_index + i)+".png")
    ...
    if opt == "SGD":
        optimizer = torch.optim.SGD([
            {'params': [param for name, param in model.named_parameters() if name[-4:] == 'bias'],
             'lr': 2 * l_rate},
            {'params': [param for name, param in model.named_parameters() if name[-4:] != 'bias'],
             'lr': l_rate, 'weight_decay': weight_decay}]
            , momentum=0.9)
    elif opt =="Adam":
        ...
        with tqdm.tqdm(trainloader, ncols=150) as t:
            ...
            for i, (images, labels) in enumerate(t):
                ...
                if i>0 and local_acc.count > 500 and processed_image % checkpoint_save == 0:
                    save_name = base_save_folder + "/checkpoint_" + str(epoch) + "_" + str(processed_image) + "_" + str(local_IoU.avg) + ".pth.tar"
                    torch.save({
                        'epoch': epoch,
...
def eval(model):
    ...
    loader_val = CityscapesLoader2('/home/cattaneod/CITYSCAPES/', split='val', img_size=None, transforms=data_augmentation_slided, return_name=True)
    ...
    os.makedirs('./CITYSCAPES/PSP/inference/'+str(runs))
    ...
    with tqdm.tqdm(loader_val, ncols=150) as t:
        for i in range(len(loader_val)):
            ...
            overlay_images([name], image, predicted_img, i, runs , tensor=False, convert_id=True)
